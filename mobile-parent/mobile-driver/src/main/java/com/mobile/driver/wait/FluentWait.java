...
import com.mobile.driver.wait.conditions.ExpectedConditions;
import com.mobile.driver.wait.exception.TimeoutException;
...
public class FluentWait<T> implements Wait<T> {
	...
	public <V> V until(Function<? super T, V> isTrue) {
		...
		while (true) {
			...
			if (!clock.isNowBefore(end)) {
				...
				String timeoutMessage = String.format("Timed out after %d seconds%s", timeout.in(TimeUnit.SECONDS), toAppend);
	...
	public <V> V until(Function<? super T, V> isTrue, Class<? extends Exception> exceptionClass) {
		long end = clock.laterBy(timeout.in(TimeUnit.MILLISECONDS));
		...
		while (true) {
			...
			if (!clock.isNowBefore(end)) {
				...
				String timeoutMessage = String.format("Timed out after %d seconds%s", timeout.in(TimeUnit.SECONDS), toAppend);
				Exception ex = buildException(exceptionClass, timeoutMessage);
	...
	private Exception buildException(Class<? extends Exception> exceptionClass, String message) {
		Constructor<? extends Exception> exConstructor;
		try {
